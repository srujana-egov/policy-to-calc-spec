...
From calculation-engine-3.0.0.yaml's /{module}/estimate description:
...
Not full engine parity (no cross-tenant registry, no persistence) — it's the "does this spec
compute what the policy document says it should" check for the synthetic-payload demo step.
"""
...
def resolve_json_path(path: str, root: dict):
    """Root-relative JSONPath (e.g. '$.tradeLicenseDetail.premisesArea') against the full payload."""
    if path.startswith("$."):
        path = path[2:]
...
def resolve_relative_path(path: str, node: dict):
    """Sub-entity-relative path (e.g. 'quantity') against one array element."""
...
def _pick_most_specific(candidates: list[dict]) -> dict | None:
    """Engine rule: when multiple RATE_MATRIX rules could match, most specific (fewest
    wildcarded, i.e. most condition keys) wins; tie-break by lower priority."""
    if not candidates:
        return None
...
def _compute_amount(rule: dict, context, derived: dict, line_items: list[dict], relative: bool) -> float:
    ...
    if calc_type == "PERCENTAGE":
        base = _amount_of(line_items, rule["appliesOn"]["componentRef"])
    ...
    if calc_type == "SLAB":
        path = rule["appliesOn"]["jsonPath"]
        field_value = resolve_relative_path(path, context) if relative else resolve_json_path(path, context)
        return _compute_slab(rule["slabs"], field_value)
    if calc_type == "FORMULA":
        variables = {}
        for name, binding in (rule.get("formulaVariables") or {}).items():
            if "componentRef" in binding and binding["componentRef"]:
                variables[name] = _amount_of(line_items, binding["componentRef"])
            else:
...
def simulate_estimate(rules: list[dict], entity_detail: dict) -> dict:
    ...
    for rule in sorted((r for r in rules if r["ruleType"] == "AGGREGATION"), key=lambda r: r.get("priority", 100)):
        ...
        derived[rule["targetAttribute"]] = result

    # 2. RATE_MATRIX, grouped by component — pick the most specific matching rule
    ...
    # 3. ADJUSTMENT, cumulative, ascending priority
    ...
    # 4. remaining PENALTY/INTEREST/TAX, dependsOn topological order
    ...
    return {"lineItems": line_items, "taxableAmount": taxable_amount, "totalAmount": total_amount}
