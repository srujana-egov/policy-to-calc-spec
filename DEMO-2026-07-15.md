...
this demo: **448 checks** (calc-engine) + tests in registry (~133) + workflow (~83) — see the
...
**Test coverage:** 25 (builder+validate) + 15 (formula parser) + 19 (worked-examples generator) +
...
| Calculation Engine | 448 | The real OpenAPI spec (`calculation-engine-3.0.0.yaml`, confirmed from the platform team) + 30 real, authoritative example rule bodies | 9 (Slab rate `/100`, AGGREGATION field absence — guessed twice — relative sub-entity paths, `derivedFrom` keying, componentRef-to-aggregation fallback, dropped `if`/comparison support, missing `/calculation/v3` path prefix, bulk-array write body instead of one-`POST`-per-rule, componentRef-to-a-non-firing-conditional-rule crash) |
