...
## 9. Suggested live demo order for tomorrow

1. **Open with the architecture diagram and the shared pattern** (§1) — one slide, "same five
   stages, three times over," before showing any tool specifically. Sets up why the walkthroughs
   will feel repetitive in a good way, and doubles as the answer if "why no AI" comes up (no
   purple/AI-colored box anywhere in the diagram).
2. **Registry first** — run the `trade-license-application` wizard live (§2), end on the summary
...
3. **Calculation Engine second** — this is the one with the best "aha" moment. Build the 5-rule tax
   stack live (§3), let the worked-example preview show ₹645 computed, not just five rule
   ...
   checkbox.
4. **Workflow third** — build `trade-license-approval` live (§4), let the diagram render, click a
   ...
   here's the fix").
5. **Close on the rigor scorecard** (§8) — the receipts for everything claimed along the way.
6. **If asked about production timeline**, go to §7's step-by-step roadmap directly — it's framed exactly for that
   question.
