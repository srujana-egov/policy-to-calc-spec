...
## 7. Rigor scorecard (evidence, not a claim)
...
## 8. Suggested live demo order for tomorrow
...
4. **Workflow third** — build `trade-license-approval` live (§4), let the diagram render, click a
   ...
   here's the fix").
5. **Close on the rigor scorecard** (§7) — the receipts for everything claimed along the way.
6. **If asked about production timeline**, go to §6's table directly — it's framed exactly for that
   question.
